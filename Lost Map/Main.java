...
public class Main {
    public static void main(String[] args) {
        ...
        Comparator<Road> roadComparator = new Comparator<>() {
            @Override
            public int compare(Road road1, Road road2) {
                return road1.distance - road2.distance;
            }
        };
        PriorityQueue<Road> roadPQ = new PriorityQueue<>(roadComparator);
        int numberOfVillages = fr.nextInt();
        for (int i = 0; i < numberOfVillages - 1; i++) {
            for (int j = 0; j < numberOfVillages; j++) {
                int distance = fr.nextInt();
                if (j > i) {
                    roadPQ.add(new Road(i, j, distance));
                }
            }
        }
        StringBuilder output = new StringBuilder();
        int numberOfConnectingRoads = 0;
        UF directVillageConnections = new UF(numberOfVillages);
        while (!roadPQ.isEmpty() && numberOfConnectingRoads < numberOfVillages) {
            Road road = roadPQ.poll();
            int start = road.start;
            int end = road.end;
            if (!directVillageConnections.connected(start, end)) {
                directVillageConnections.union(start, end);
                numberOfConnectingRoads++;
