...
public class Main {
    public static void main(String[] args) {
        ...
        int n = fio.nextInt();
        IndexFibonacciMinPQ<Cat> cats = new IndexFibonacciMinPQ<>(1000000);
        HashMap<String, Cat> infectionLevels = new HashMap<>();
        int count = 0;
        for (int i = 0; i < n; i++) {
            ...
            if (query == 0) {
                ...
                Cat cat = new Cat(name, infectionLevel, count);
                cats.insert(count, cat);
                infectionLevels.put(name, cat);
                count++;
            } else if (query == 1) {
                String name = fio.next();
                ...
                if (increase > 0) {
                    Cat cat = infectionLevels.get(name);
                    Cat newCat = new Cat(name, cat.infectionLevel + increase, cat.arrivalTime);
                    cats.decreaseKey(cat.arrivalTime, newCat);
                    infectionLevels.put(name, newCat);
                }
            } else if (query == 2) {
                String name = fio.next();
                Cat cat = infectionLevels.get(name);
                if (cats.size() == 1) {
                    ...
                    cats.delete(cat.arrivalTime);
                }
            } else {
                if (cats.isEmpty()) {
                    fio.println("The clinic is empty");
                } else {
                    fio.println(cats.minKey().name);
                }
            }
        }
        fio.close();
...
class Cat implements Comparable<Cat> {
    ...
    public int compareTo(Cat other) {
        if (this.infectionLevel != other.infectionLevel) {
            return other.infectionLevel - this.infectionLevel;
        }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(name, infectionLevel, arrivalTime);
    }

}

/******************************************************************************
 *  Source: https://algs4.cs.princeton.edu/99misc/IndexFibonacciMinPQ.java.html
 *
 ...
 */
class IndexFibonacciMinPQ<Key> implements Iterable<Integer> {
